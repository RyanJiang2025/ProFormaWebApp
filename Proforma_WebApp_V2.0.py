...
API_URL = os.getenv("API_URL", "http://127.0.0.1:50053")
ENDPOINT = "simulate"
...
def rank_to_profit(rank):
    return (
        rank * 0.2
    )  # We changed it here from a quadratic to a linear function. Also, right now, a ranking under 5 means that the developer gets less MRU to "break even" (based on our back of envelope calculations). A 10 is 100% extra profit, 1 is almost 0.
...
def GPTOutputTable_Builder(
    Item_Accounting_table, MRU_Add_Table, min_units, max_units, step, item
):
...
def generate_gpt_tables(
    Item_Accounting_table, MRU_Add_Table
